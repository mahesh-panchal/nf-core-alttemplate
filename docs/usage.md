# nf-core/alttemplate: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/alttemplate/usage](https://nf-co.re/alttemplate/usage)
...
```console
nextflow run nf-core/alttemplate --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/alttemplate
```
...
First, go to the [nf-core/alttemplate releases page](https://github.com/nf-core/alttemplate/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
